...
function MessLeft() {
  ...
  const [findProfils, setfindProfils] = useState([])
  function FindProfilFunc(nick) {
    const findw = Users.filter((i) => nick === i.nickname || nick === i.profilname)
    if (findw.lenght > 0) {
      setfindProfils([]);
       
    }
    else {
      setfindProfils(findw)
      // alert('!error')
    }
  }
    ...
    <div className="leftMenu" style={{ paddingRight: "10px", paddingLeft: "10px", float: "left", width: "auto", borderRight: "solid 1px", borderRightColor: "grey" }}>
        ...
        <div className="row">
          ...
          <div className="col-12 mt-4 border-bottom pb-4">
            <div className="form-group modalinpblock">
              <input onChange={(e) => FindProfilFunc(e.target.value)} type="text" className="inpmol rounded" placeholder="Поиск" /><i class="fa-solid fa-circle-xmark fa-lg" style={{ cursor: "pointer" }}></i>
            </div>
          </div>
          <div className="col-12 FindScroll mt-2" style={{height:"600px"}}>
            <div className="row">
      ...
      <div>
        <ul className="p-2" style={{ fontWeight: "400" }}>
          ...
          <li className="mt-2">
            <button className='iconButton' style={{ textAlign: "center", paddingRight: "7px" }}><i class="fa-brands fa-facebook-messenger fa-xl"></i></button>
          </li>
          <li className="mt-2">
